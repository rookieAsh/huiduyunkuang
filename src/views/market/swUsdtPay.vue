<template>
  <div class="content margin0 ">
    <div class="flex-between">
      <div class="leftContent">
        ...
        <div class="balance" v-if="this.active == 0">
          <div class="balValue" v-if="balance">账户余额：{{ balance }}</div>
          <div class="balValue" v-else>账户余额：0.000000</div>
    ...
    >
      ...
      <div style="font-size:20px"><input type="file" id="doc" /></div>
  ...
  methods: {
    ...
    getDetail() {
      ...
      this.$axios.post("/swarm/selectSwarmByProductId", param).then(res => {
        ...
        this.picImage = obj.logo;
        this.picName = obj.cmc_name;
      });
    },
    // 获得银行卡的信息
    ...
    // 上传凭证
    handleCredentials() {
      if (this.bankcardValue == "") {
        this.$message.warning("请选择转账银行卡");
        return;
      }
    ...
    handleChange(e) {
      console.log(e);
      if (e == "杭州爱挖云科技有限公司") {
        this.amounts = Number(this.amounts) + Number(this.amounts * 0.06);
      } else {
        this.amounts = this.$route.query.cost;
      }
    },
    //  银行卡支付
    cloudPay(value) {
      //   if (this.bankcardValue == "杭州爱挖云科技有限公司") {
    ...
    getBalance() {
      ...
      this.$axios.post("custody/selectWellatByCoinFlash", param).then(res => {
        ...
        result.map(item => {
          if (item.coin == "USDT") {
            this.urlAddressKey = item.addressKey;
