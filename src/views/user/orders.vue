    ...
    <div class="ordersPage">
        ...
        <div class="content">
            ...
            <div class="item" v-for="(item,index) in list" :key="index" >
                <div class="flex">
                    <div class="pic">
                        <img src="../../assets/imgs/coupon.png" style="width:240px;height:200px">
                    </div>
                    <div class="info">
                        <div class="name">{{item.product_name}}</div>
                        <div class="th flex">
                            <div class="li">X{{item.number}}</div>
                            <div class="li">{{item.payment_method==1?"币币交易":"银行卡支付"}}</div>
                ...
                <div class="flex-between" style="paddingTop:30px">
                    <div class="oNum">订单编号 {{item.order_number}} <i class="el-icon-copy-document"></i> </div>
                    <div class="state" style="color: #F16C00;">{{['已支付','审核中','购买失败'][item.zt]}}</div>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods:{
        init(){
            ...
            this.$axios.post("/MartianOrePool/selectOrderByAddress", param).then((res) => {
                let result = res.data;
                if (result.state == 0) {
